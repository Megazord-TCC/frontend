...
import { UserStatus, UserStatusEnumDTO } from "../../interface/carlos-user-interfaces";
...
const getBadgeConfigurations = (): BadgeConfiguration[] => {
    ...
    badgeConfig.triggeringValues = [UserStatus.ACTIVE];
    badgeConfigs.push(badgeConfig);

    badgeConfig = new BadgeConfiguration();
    badgeConfig.color = 'gray';
    badgeConfig.triggeringValues = [UserStatus.INACTIVE];
    badgeConfigs.push(badgeConfig);
...
export const getColumns = (): TableColumn[] => {
    ...
    column.order = 2;
    column.isSortable = true;
    column.frontendAttributeName = 'email';
    column.backendAttributeName = 'email';
    columns.push(column);
    ...
    column.order = 3;
    column.isSortable = true;
    column.frontendAttributeName = 'role';
    column.backendAttributeName = 'role';
    columns.push(column);
...
export const getFilterText = (): InputFilter => {
    ...
    input.label = 'Buscar pelo nome do usuário';
...
export const getFilterButtons = (): InputFilter[] => {
    ...
    input.label = 'Ativado';
    input.queryParam = { name: 'status', value: UserStatusEnumDTO.ACTIVE };
    inputs.push(input);

    input = new InputFilter();
    input.label = 'Desativado';
    input.queryParam = { name: 'status', value: UserStatusEnumDTO.INACTIVE };
    inputs.push(input);
